...
namespace Shuttle.Core.Configuration
{
    public class ConfigurationSectionProvider
    {
        ...
        {
            return (T) ConfigurationManager.OpenMappedMachineConfiguration(new ConfigurationFileMap(file))
                .GetSection(name);
        }
        ...
        public static T OpenFile<T>(string group, string name, string file) where T : ConfigurationSection
        {
            ...
            return (T) (sectionGroup == null || sectionGroup.Sections[name] == null
                ? configuration.GetSection(name)
                : sectionGroup.Sections[name]);
        }
    }
}